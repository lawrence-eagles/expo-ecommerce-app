...
const ProfileScreen = () => {
  ...
  const handleMenuPress = (action: (typeof MENU_ITEMS)[number]["action"]) => {
    if (action === "/edit-profile") return; // the profile page does not work. But I can build it latter
    router.push(action);
  };
  ...
  return (
    <SafeScreen>
      ...
      >
        ...
        {/* MENU ITEMS */}
        <View className="flex-row flex-wrap gap-2 mx-6 mb-3 justify-center">
          {MENU_ITEMS.map((item) => (
            <TouchableOpacity
              key={item.id}

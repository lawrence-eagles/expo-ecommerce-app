...
const OrdersScreen = () => {
  ...
  const handleOpenRating = (order: Order) => {
    ...
    order.orderItems.forEach((item) => {
      const productId = item.product._id; // if error let this be item.product
  ...
  const handleSubmitRating = async () => {
    ...
    if (!allRated) {
      Alert.alert("Error", "Pleae rate all products");
      return;
    }
    ...
    try {
      await Promise.all(
        selectedOrder.orderItems.map((item) => {
          createReviewAsync({
            productId: item.product._id, // if error let this be item.product

            orderId: selectedOrder._id,
            rating: productRatings[item.product._id], // if error let this be item.product
          });
        }),
      );
...
function ErrorUI() {
  return (
    <View className="flex-1 items center justify-center px-6">
      ...
      <Text className="text-text-secondary text-center mt">
